...
```sh
curl -fsSL https://github.com/cfpy67/au-cli/releases/download/v0.3.7-alpha/au-linux-amd64 -o au && chmod +x au && sudo mkdir -p /usr/local/bin && sudo mv au /usr/local/bin/au
```

For ARM64 (Raspberry Pi, Ampere VPS):

```sh
curl -fsSL https://github.com/cfpy67/au-cli/releases/download/v0.3.7-alpha/au-linux-arm64 -o au && chmod +x au && sudo mkdir -p /usr/local/bin && sudo mv au /usr/local/bin/au
```
...
```sh
curl -fsSL https://github.com/cfpy67/au-cli/releases/download/v0.3.7-alpha/au-darwin-amd64 -o au && chmod +x au && sudo mkdir -p /usr/local/bin && sudo mv au /usr/local/bin/au
```

Apple Silicon (M1/M2/M3):

```sh
curl -fsSL https://github.com/cfpy67/au-cli/releases/download/v0.3.7-alpha/au-darwin-arm64 -o au && chmod +x au && sudo mkdir -p /usr/local/bin && sudo mv au /usr/local/bin/au
```
...
Download [`au-windows-amd64.exe`](https://github.com/cfpy67/au-cli/releases/download/v0.3.7-alpha/au-windows-amd64.exe), rename it to `au.exe`, and place it somewhere on your `PATH` (e.g. `C:\Windows\System32` or any folder in your user PATH).
